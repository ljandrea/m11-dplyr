...
arr.delay.dest <- flights %>% group_by(dest) %>% summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>% 
...
arr.delay.airline <- flights %>% group_by(carrier) %>% summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>% 
